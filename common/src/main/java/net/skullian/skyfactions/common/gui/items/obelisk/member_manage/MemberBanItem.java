...
public class MemberBanItem extends SkyItem {
    ...
    public void onClick(@NotNull ClickType clickType, @NotNull Player player, @NotNull InventoryClickEvent event) {
        ...
        if (faction.getAllMembers().contains(SUBJECT)) {
            ...
            Messages.FACTION_MANAGE_BAN_SUCCESS.send(player, SpigotPlayerAPI.getLocale(player.getUniqueId()), "player", SUBJECT.getName());
        } else {
            Messages.ERROR.send(player, SpigotPlayerAPI.getLocale(player.getUniqueId()), "operation", "ban a player", "debug", "FACTION_MEMBER_UNKNOWN");
            event.getInventory().close();
        }
    }
