...
public class GUIAPI {
    ...
    public static GUIData getGUIData(String guiName, Player player) throws IllegalArgumentException {
        YamlDocument config = GUIEnums.configs.getOrDefault(SpigotPlayerAPI.getLocale(player.getUniqueId()), getFallbackLanguage()).get(guiName);
        if (config != null) {
    ...
    public static List<PaginationItemData> getPaginationData(Player player) throws RuntimeException {
        YamlDocument config = GUIEnums.configs.getOrDefault(SpigotPlayerAPI.getLocale(player.getUniqueId()), getFallbackLanguage()).get("pagination");
        if (config != null) {
    ...
    private static ItemStack create(String material, String texture, UUID playerUUID) {
        ...
        if (material.equalsIgnoreCase("PLAYER_HEAD")) {
            if (texture.equalsIgnoreCase("<player_skull>")) {
                stack = SpigotPlayerAPI.getPlayerSkull(new ItemStack(Material.PLAYER_HEAD), playerUUID);
            } else {
                stack = SpigotPlayerAPI.convertToSkull(new ItemStack(Material.PLAYER_HEAD), texture);
            }
        } else {
