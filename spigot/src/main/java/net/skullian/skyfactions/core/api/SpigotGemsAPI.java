...
public class SpigotGemsAPI extends GemsAPI {
    ...
    public ItemStack createGemsStack(Player player) {
        ...
        if (!Messages.GEMS_ITEM_LORE.getStringList(locale).isEmpty()) meta.lore(
                Messages.GEMS_ITEM_LORE.getStringList(locale).stream()
                        .map(text -> TextUtility.color(text, locale, player))
