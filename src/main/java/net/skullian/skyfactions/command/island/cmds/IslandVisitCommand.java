...
public class IslandVisitCommand extends CommandTemplate {
    ...
    public void perform(
        ...
        Messages.VISIT_PROCESSING.send(player, PlayerAPI.getLocale(player.getUniqueId()));
        ...
        PlayerAPI.isPlayerRegistered(target.getUniqueId()).whenComplete((isRegistered, throwable) -> {
            ...
            IslandAPI.getPlayerIsland(target.getUniqueId()).whenComplete((is, ex) -> {
                if (ex != null) {
                    ...
                    return;
                } else if (RegionAPI.isLocationInRegion(player.getLocation(), "sfr_player_" + target.getUniqueId().toString())) {
                ...
                if ((RaidAPI.currentRaids.containsValue(player.getUniqueId()) || RaidAPI.processingRaid.containsValue(player.getUniqueId())) || (RaidAPI.currentRaids.containsValue(target.getUniqueId()) || RaidAPI.processingRaid.containsValue(target.getUniqueId()))) {
                    Messages.VISIT_IN_RAID.send(player, PlayerAPI.getLocale(player.getUniqueId()));
                } else {
                    SkyFactionsReborn.getDatabaseManager().getPlayerIslandManager().isPlayerTrusted(player.getUniqueId(), is.getId()).whenComplete((isTrusted, err) -> {
                        ...
                        if (world == null) {
                            Messages.ERROR.send(player, PlayerAPI.getLocale(player.getUniqueId()), "operation", "visit a player", "debug", "WORLD_NOT_EXIST");
                        } else {
                            if (isTrusted) {
                                ...
                                Messages.PLAYER_NOT_TRUSTED.send(player, PlayerAPI.getLocale(player.getUniqueId()));
                            }
                        }
                    });
                }
