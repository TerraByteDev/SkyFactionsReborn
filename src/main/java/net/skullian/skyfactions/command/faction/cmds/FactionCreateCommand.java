...
public class FactionCreateCommand extends CommandTemplate {
    ...
    public void perform(
        ...
        Messages.FACTION_CREATION_PROCESSING.send(player, player.locale().getLanguage());
        ...
        FactionAPI.isInFaction(player).whenComplete((isInFaction, ex) -> {
            ...
            if (isInFaction) {
                ...
                FactionAPI.getFaction(name).whenComplete((faction, exc) -> {
                    ...
                    if (faction != null) {
                        Messages.FACTION_CREATION_NAME_DUPLICATE.send(player, player.locale().getLanguage());
                    } else {
                        if (FactionAPI.hasValidName(player, name)) {
                            ...
                            if (cost > 0) {
                                ...
                                if (runes < cost) {
                                    Messages.FACTION_INSUFFICIENT_FUNDS.send(player, player.locale().getLanguage(), "%creation_cost%", cost);
                                } else {
