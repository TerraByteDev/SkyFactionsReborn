...
public class RaidAPI {
    ...
    public static CompletableFuture<String> getCooldownDuration(Player player) {
        ...
        return SkyFactionsReborn.databaseManager.playerManager.getLastRaid(player).thenApply((lastTime) -> {
    ...
    public static void startRaid(Player player) {
        ...
        try {
            ...
            if (island != null) {
                ...
                Messages.ERROR.send(player, PlayerHandler.getLocale(player.getUniqueId()), "operation", "start a raid", "debug", "ISLAND_RETURNED_NULL");
            }

        } catch (Exception error) {
            error.printStackTrace();
            Messages.ERROR.send(player, PlayerHandler.getLocale(player.getUniqueId()), "operation", "start a raid", "debug", "MAIN_RAID_START");
            handleRaidExecutionError(player, false);
        }
    }
    ...
    private static void handlePlayers(Player attacker, UUID uuid) {
        ...
        if (!isPlayerOnline(uuid)) {
            ...
            IslandAPI.getPlayerIsland(def.getUniqueId()).thenAccept(is -> SkyFactionsReborn.databaseManager.playerIslandManager.setIslandCooldown(is, System.currentTimeMillis())).exceptionally(ex -> {
                cancel.set(true);
    ...
    public static boolean hasEnoughGems(Player player) {
        try {
            ...
            SkyFactionsReborn.databaseManager.currencyManager.getGems(player.getUniqueId()).thenAccept(currentGems::set).exceptionally(ex -> {
                ex.printStackTrace();
            ...
            if (currentGems.get() < required) {
                Messages.RAID_INSUFFICIENT_GEMS.send(player, PlayerHandler.getLocale(player.getUniqueId()), "raid_cost", required);
                return false;
            } else {
                return true;
            }
            ...
            error.printStackTrace();
            Messages.ERROR.send(player, PlayerHandler.getLocale(player.getUniqueId()), "operation", "start a raid", "debug", "SQL_GEMS_GET");
        }
    ...
    public static IslandRaidData getRandomRaidable(Player player) {
        try {
            ...
            SkyFactionsReborn.databaseManager.playerIslandManager.getRaidableIslands(player).thenAccept(data::set).exceptionally(ex -> {
                ex.printStackTrace();
            ...
            if (data.get().isEmpty()) {
                Messages.RAID_NO_PLAYERS.send(player, PlayerHandler.getLocale(player.getUniqueId()));
            } else {
    ...
    private static void alertPlayer(Player player, Player attacker) {
        ...
        List<String> alertList = Messages.RAID_IMMINENT_MESSAGE.getStringList(PlayerHandler.getLocale(player.getUniqueId()));

        if (player.isOnline()) {
            for (String msg : alertList) {
                player.sendMessage(TextUtility.color(msg.replace("player_name", player.getName()).replace("raider", attacker.getName()), PlayerHandler.getLocale(player.getUniqueId()), player));
            }
        }
    }
    ...
    private static void handleRaidExecutionError(Player player, boolean isDefendant) {
        ...
        SkyFactionsReborn.databaseManager.playerManager.updateLastRaid(player, 0).exceptionally(ex -> {
            ex.printStackTrace();
            Messages.ERROR.send(player, PlayerHandler.getLocale(player.getUniqueId()), "operation", "handle raid errors", "debug", "SQL_RAID_UPDATE");
            return null;
        });
        SkyFactionsReborn.databaseManager.currencyManager.modifyGems(player.getUniqueId(), Settings.RAIDING_COST.getInt(), false).join();

        if (isDefendant) {
            IslandAPI.getPlayerIsland(player.getUniqueId()).thenAccept(island -> SkyFactionsReborn.databaseManager.playerIslandManager.setIslandCooldown(island, 0).exceptionally(ex -> {
                ex.printStackTrace();
                ...
                ex.printStackTrace();
                Messages.ERROR.send(player, PlayerHandler.getLocale(player.getUniqueId()), "operation", "handle raid errors", "debug", "SQL_ISLAND_GET");
                return null;
            });
        }
    }
