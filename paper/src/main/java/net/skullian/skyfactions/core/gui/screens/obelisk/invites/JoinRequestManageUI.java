...
public class JoinRequestManageUI extends Screen {
    ...
    protected Item handleItem(@NotNull ItemData itemData) {
        return switch (itemData.getITEM_ID()) {
            ...
            case "REJECT" ->
                    new FactionJoinRequestRejectItem(itemData, SpigotGUIAPI.createItem(itemData, player.getUniqueId()), inviteData, player);
            case "ACCEPT" ->
                    new FactionJoinRequestAcceptItem(itemData, SpigotGUIAPI.createItem(itemData, player.getUniqueId()), inviteData, player);
            default -> null;
        };
    }
}
