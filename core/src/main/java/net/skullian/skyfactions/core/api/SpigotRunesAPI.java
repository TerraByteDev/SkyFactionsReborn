...
public class SpigotRunesAPI extends RunesAPI {
    ...
    public boolean isStackProhibited(ItemStack stack, Player player) {
        if (!RunesConfig.ALLOW_ENCHANTS.getBoolean() && hasEnchants(stack)) {
            Messages.RUNE_ENCHANT_DENY.send(player, SpigotPlayerAPI.getLocale(player.getUniqueId()));
            return true;
        } else if (!RunesConfig.ALLOW_LORE.getBoolean() && hasLore(stack)) {
            Messages.RUNE_GENERAL_DENY.send(player, SpigotPlayerAPI.getLocale(player.getUniqueId()));
            return true;
    ...
    public void handleFactionRuneConversion(List<ItemStack> stacks, Player player) {
        FactionAPI.getFaction(player.getUniqueId()).whenComplete((faction, ex) -> {
            ...
            if (faction != null) {
                ...
                Messages.ERROR.send(player, SpigotPlayerAPI.getLocale(player.getUniqueId()), "operation", "convert to runes", "debug", "SQL_UNKNOWN_FACTION");
    ...
    public void handleRuneConversion(List<ItemStack> stacks, Player player, Faction faction) {
        ...
        if (total > 0) {
            ...
            Messages.RUNE_CONVERSION_SUCCESS.send(player, SpigotPlayerAPI.getLocale(player.getUniqueId()), "added", total);
        } else {
            Messages.RUNE_INSUFFICIENT_ITEMS.send(player, SpigotPlayerAPI.getLocale(player.getUniqueId()));
        }
    }
    ...
    public CompletableFuture<Integer> getRunes(UUID playerUUID) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            if (SkyApi.getInstance().getCacheService().getPlayersToCache().containsKey(playerUUID) && playerRunes.containsKey(playerUUID)) return (playerRunes.getOrDefault(playerUUID, 0) + SkyApi.getInstance().getCacheService().getPlayersToCache().get(playerUUID).getRunes());
                else return playerRunes.getOrDefault(playerUUID, 0);
        });
    }

    @Override
    public int getRunesIfCached(UUID playerUUID) {
        if (!playerRunes.containsKey(playerUUID)) cachePlayer(playerUUID);
    ...
    public boolean isPlayerCached(UUID playerUUID) {
        return playerRunes.containsKey(playerUUID);
    }

    @Override
    public CompletableFuture<Integer> cachePlayer(UUID playerUUID) {
        if (playerRunes.containsKey(playerUUID)) return CompletableFuture.completedFuture(getRunesIfCached(playerUUID));
        return SkyApi.getInstance().getDatabaseManager().getCurrencyManager().getRunes(playerUUID).whenComplete((runes, ex) -> {
            if (ex != null) {
                ex.printStackTrace();
                return;
            }
